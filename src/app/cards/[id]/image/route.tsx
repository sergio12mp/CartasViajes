...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const card = await prisma.cardType.findUnique({ where: { id }, include: { pack: { select: { name: true } } } });
  ...
  return new ImageResponse(
    <Frame style={{ padding: 60 }}>
      <div style={{ display: "flex", flexDirection: "column", flex: 1, borderRadius: 48, border: `18px solid ${color}`, background: colors.surface, padding: 64, ...(card.rarity === "LEGENDARY" ? { boxShadow: `0 0 0 14px ${color}44` } : {}) }}>
        ...
        <div style={{ display: "flex", fontSize: 34, color: colors.soft, textTransform: "capitalize" }}>{card.category}{card.pack ? ` · ${card.pack.name}` : ""}</div>
