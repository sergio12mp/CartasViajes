...
export type CardTypeView = { id?: string; name: string; description: string; category: string; kind: "ATTACK" | "REACTION"; rarity: RarityKey; emoji: string | null; creditName?: string | null };
...
export function CardTile({ card, onPlay, mode = "hand" }: { card: HandCard; onPlay?: () => void; mode?: "hand" | "preview" }) {
  ...
  const content = <><div className="flex items-center justify-between gap-2"><span className="text-3xl" aria-hidden>{cardType.emoji ?? "🃏"}</span><span className="text-[10px] font-bold uppercase tracking-widest">{used ? "🔒 Usada" : cardType.kind === "ATTACK" ? "Ataque" : "Reacción"}</span></div>
    <div className="space-y-2"><div className="flex flex-wrap items-center gap-2"><span className={rarityBadgeClass[cardType.rarity]}>{rarityLabels[cardType.rarity]}</span><span className="text-[10px] capitalize text-muted">{cardType.category}</span></div><p className="text-base font-bold leading-tight">{cardType.name}</p><p className="text-xs leading-relaxed">{cardType.description}</p>{cardType.creditName && <p className="text-[10px] text-muted">Propuesta por {cardType.creditName}</p>}</div>
    {mode === "hand" ? <p className="mt-auto pt-2 text-[11px] font-semibold">{used ? "Carta usada" : cardType.kind === "ATTACK" ? "Toca para jugar ↗" : "Se usa cuando te ataquen"}</p> : cardType.id && <a href={`/cards/${cardType.id}/image`} target="_blank" rel="noopener" className="mt-auto pt-2 text-[11px] font-semibold text-primary">Ver tarjeta ↗</a>}</>;
