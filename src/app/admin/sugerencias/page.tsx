...
export default async function AdminSuggestionsPage() {
  await requireAdmin();
  const suggestions = await prisma.cardSuggestion.findMany({ include: { user: { select: { name: true } } }, orderBy: [{ status: "desc" }, { createdAt: "desc" }], take: 200 });
  ...
  const list = (items: typeof suggestions) => items.length === 0 ? <p className="text-sm text-muted">Nada por aquí.</p> : <ul className="space-y-3">{items.map(s => <li key={s.id} className="panel space-y-2">
    ...
    <p className="text-xs text-muted">De {s.user.name ?? "alguien"}</p>
    <ActionForm action={setSuggestionStatus} fields={{ suggestionId: s.id, status: s.status === "NEW" ? "REVIEWED" : "NEW" }} label={s.status === "NEW" ? "Marcar como revisada" : "Reabrir"} secondary />
  </li>)}</ul>;
