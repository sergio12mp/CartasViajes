...
export function PendingPlays({ tripId, incoming, outgoing, reactions }: { tripId: string; incoming: PendingPlayView[]; outgoing: PendingPlayView[]; reactions: HandCard[] }) {
  ...
  return <section className="space-y-4"><h2>En la mesa</h2>
    ...
    {incoming.map(play => <article className="rounded-2xl border-2 border-warning bg-accent/10 p-5" key={play.id}>
      ...
      <div className="flex flex-wrap gap-2"><ActionForm action={acceptPlay} fields={{ tripId, playId: play.id }} label="Aceptar" disabled={seconds(play) === 0} />
        {reactions.map(card => <ActionForm key={card.id} action={reactToPlay} fields={{ tripId, playId: play.id, reactionCardId: card.id }} label={`Usar ${card.cardType.name}`} disabled={seconds(play) === 0} secondary />)}
      </div>
    </article>)}
