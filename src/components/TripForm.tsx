...
export function TripForm({ cards, action, initialValues }: { cards: CardType[]; action: FormAction; initialValues?: TripFormValues }) {
  ...
  const activeCategories = categories.filter(category => nonLegendary.some(c => c.category === category));
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <section className="panel space-y-5"><div><h2>03 · El reparto</h2><p className="mt-1 text-sm text-ink-soft">Cada jugador recibe sus legendarias (nunca se repiten en el viaje) y el resto se completa al azar.</p></div>
      ...
      <p className={`rounded-xl p-3 text-sm ${legendariesInPool >= legendariesNeeded ? "bg-bg" : "bg-danger/10 text-danger"}`}>{legendariesInPool} legendarias en el mazo · hacen falta {legendariesNeeded} ({players.length} jugadores × {legendaries}).{legendariesInPool < legendariesNeeded && " Añade legendarias o reduce las legendarias por jugador."}</p>
