...
export default async function CommunityPage() {
  ...
  const [videos, mine, trips] = await Promise.all([getApprovedVideos(), userId ? getVideosForUser(userId) : [], userId ? getTripsForUser(userId) : []]);
  const destinations = [...new Set(videos.map(v => v.destination).filter((d): d is string => Boolean(d)))];
  return <div className="space-y-8">
    <div><p className="text-xs font-bold uppercase tracking-[0.25em] text-primary">Comunidad</p><h1 className="mt-2">Así se juega por el mundo</h1><p className="mt-2 text-ink-soft">TikToks de viajes jugando a CartasViajes. Etiqueta a @CartasViajes o usa #CartasViajes y envíanos el enlace.</p></div>
