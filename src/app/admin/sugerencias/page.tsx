...
export default async function AdminSuggestionsPage() {
  ...
  const list = (items: typeof suggestions) => items.length === 0 ? <p className="text-sm text-muted">Nada por aquí.</p> : <ul className="space-y-3">{items.map(s => <li key={s.id} className="panel space-y-2">
    <div className="flex flex-wrap justify-between gap-2"><span className="font-semibold">{s.name}</span><time className="text-xs text-muted" dateTime={s.createdAt.toISOString()}>{formatDate(s.createdAt)}</time></div>
    <div className="flex flex-wrap items-center gap-2"><span className="badge">{getCategoryPresentation(s.category).emoji} {getCategoryPresentation(s.category).label}</span>{s.rarity ? <span className={rarityBadgeClass[s.rarity]}>{rarityLabels[s.rarity]}</span> : <span className="text-xs text-muted">Sin rareza propuesta</span>}</div>
